...
void Armo::trtSousBalise(subRecord* sousEnreg, char* bufferChar){
	if (!memcmp("NAME", sousEnreg->name, 4)) {
	...
	} else if (!memcmp("AODT", sousEnreg->name, 4)) {
		memcpy(&this->armourdata, bufferChar, sousEnreg->size);
		...
		cout << "Erreur ARMO subEnreg " << sousEnreg->name << endl;
	}
}

void Armo::openIndx(subRecord* sousEnreg, char* bufferChar){
	if (tmpBdyPartOpen){
		closeIndx(sousEnreg,bufferChar);
	}
	tmpBdyPartOpen = true;
...
void Armo::closeIndx(subRecord* sousEnreg, char* bufferChar){
	if (tmpBdyPartOpen){
